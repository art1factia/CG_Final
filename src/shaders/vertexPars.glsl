...
float fit (float unscaled, float originMin, float originMax, float scaledMin, float scaledMax) {
  return (scaledMax - scaledMin)* (unscaled- originMin) / (originMax - originMin) + scaledMin;
}

float wave(vec3 position) {
 return fit(smoothMod(position.y * 6., 1.0, 1.0), 0.3, 0.6, 0., 1.);
}
