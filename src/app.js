...
const startApp = () => {
  ...
  addPass(new UnrealBloomPass(new THREE.Vector2(width, height), 0.5, 0.4, 0.1))
  ...
  useTick(({ timestamp, timeDiff }) => {
    ...
    box1.rotation.x += time / 2000
    box1.rotation.y += time / 2000
  ...
  function onMouseClick(event) {
    ...
    if (pickHelper.pickedObject) {
      const targetPosition = new THREE.Vector3().copy(pickHelper.pickedObject.position).add(new THREE.Vector3(-2, 2*pickHelper.pickedObject.scale.y, 3));
